<template>
  <div class="slot-machines">
    <div
      v-for="(list, index) in numbers"
      ref="list"
      :key="index"
      class="list"
      :class="{'animal': listAnimal}"
      ...
      @mousedown.prevent="e => handleMouseStart(e, index)"
      @mousemove.prevent="e => handleMouseMove(e, index)"
      @mouseup.prevent="e => handleMouseEnd(e, index)"
    >
      <div v-for="(item, i) in itemlength" ref="item" :key="i" class="item" />
    </div>
  </div>
</template>
  ...
  methods: {
    init() {
      ...
      this.$refs.item.forEach((item, index) => {
        const result = this.setStyle(index)
    ...
    setStyle(index) { // 根据每个item的高度和对应的角度计算偏移量
      ...
      const zLength = this.tzLength(itemHeight / 2, this.perAngle / 2)
      return {
    ...
    getCurrentNumsByAngle(angle) { // 根据角度获取对应的数字
      const temp = angle % 360 >= 0 ? angle % 360 / this.perAngle : (360 + angle % 360) / this.perAngle
      return temp % 10
    },
    ...
    numbersByRandom() { // 提供给外部调用随机生成号码功能
      ...
      this.roundAnimal = !this.roundAnimal
      const newNumbers = [0, 0, 0, 0, 0].map(() => {
        return Math.floor(Math.random() * 10)
      })
      this.listAnimal = true
      this.$refs.list.forEach((item, index) => {
        item.style.transitionDuration = this.animalTime + index * 0.2 + 's'
        const current = this.getCurrentAngle(index)
        const MoveAngle = (newNumbers[index] - this.numbers[index]) * this.perAngle + 720 + current
        this.setListTransform(index, MoveAngle)
      })
      ...
      this.updateTimeout = setTimeout(() => { // 延时返回数据，增强交互性
        this.updateNumbers()
        this.roundAnimal = !this.roundAnimal
      }, this.animalTime * 1000 + 800)
    },
    handleMouseStart(e, index) { // 鼠标事件，移动端删除
    ...
    handleMouseMove(e, index) { // 鼠标事件，移动端删除
      if (this.touchStatus) {
        this.endY = e.clientY
        const MoveY = (this.endY - this.startY) / 1
        const MoveAngle = this.startAngle + MoveY * -1 / this.perAngleH
        this.setListTransform(index, MoveAngle)
      }
    },
    handleMouseEnd(e, index) { // 鼠标事件，移动端删除
      const currentAngle = this.getCurrentAngle(index)
      const MoveAngle =
